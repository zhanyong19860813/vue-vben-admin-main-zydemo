...
// 刷新 用户表格数据
const reloadUserGrid = async () => {
  try {
    gridApi.query();
  } catch (error) {
    console.error('刷新用户表格数据出错:', error);
    message.error('刷新用户表格数据出错');    
  }
};
...
function openAddMenuFormModal() {
  ...
  formMenuModalApi.setData({
      // 表单值
      values: {
        role_id: currentRoleId.value  
        , onSuccess: reloadMenuGrid
      }
    })
    .open();
}
...
// 刷新 菜单表格数据
const reloadMenuGrid = async () => {
  try {
    gridMenuApi.query();
  } catch (error) {
    console.error('刷新菜单表格数据出错:', error);
    message.error('刷新菜单 表格数据出错');    
  }
};
...
const handleDeleteMenu = async () => {
  try { 
    ...
    const gridmenu = gridMenuApi.grid;
    if (!gridmenu) {
      message.error('表格实例未初始化');
      return;
    }
    const rows = gridmenu.getCheckboxRecords(); // ✅ vxe-table 原生方法
      ...
      <Tabs v-model:activeKey="activeKey" :destroyInactiveTabPane="false">
        ...
        <Tabs.TabPane key="user" tab="用户列表">
          <div class="p-2">
            <div class="flex flex-col gap-2">
              <div class="mb-2 font-bold text-lg">
                <Button class="mr-1" type="primary" @click="openAddUserFormModal">
                  新增
                </Button>
                <Button danger @click="handleDeleteUser">
        ...
        <Tabs.TabPane key="role" tab="菜单权限">
          <div class="p-2">
            <div class="flex flex-col gap-2">
              <div class="mb-2 font-bold text-lg">
                <Button class="mr-1" type="primary" @click="openAddMenuFormModal">
                  新增
                </Button>
                <Button danger @click="handleDeleteMenu">
                  删除
                </Button>
              </div>
