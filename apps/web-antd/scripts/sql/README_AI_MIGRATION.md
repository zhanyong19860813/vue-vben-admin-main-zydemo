...
6. **`1008_migrate_full_考勤日结果.sql`** — **考勤日结果**（`v_att_lst_DayResult`）：只读列表 + 导出/刷新；`grid.columns` 覆盖视图主要字段（与老 datagrid 列量接近）；字段名须与当前库视图一致（见 README「Must resolve」）。`toolbarConfig.custom: true` 可列设置显隐。固定叶子 `…036`、列表 `…236`；自动删除 meta 指向 `v_att_lst_DayResult` 的旧叶子。前置同 1002（`030`）。
